...
由于涉及自动缩放系统建议的文献很多，我们试图把它整理成一些秩序，以便更好地理解和比较这些建议。在这个意义上，我们需要一些分类规则，将作品分成有意义的集合。就我们所知，以前没有任何工作提出这样的分类。最密切相关的调查是由Guitart等人完成的[61]，其目标是一般互联网应用的性能，部署在共享或专用集群上，依靠诸如准入控制和服务差异化的方法。然而，本综述的重点是利用云系统固有的弹性性质。Manvi和Shyam[78]收集了许多关于IaaS环境下资源管理的参考文献，但很少关注自动扩展问题。

我们修订了这份调查报告，在用于实现自动伸缩器的基础理论或技术方面，包括用于决定何时伸缩或需要多少资源的方法或模型，都非常不同。每种自动扩展方法都有特定的目标，专注于几种应用架构或提供不同扩展能力的云供应商。最有区别的因素是用于特定自动扩展器的最终目标/评价标准：预测精度、对SLA的遵守（以多种方式定义，如响应时间或资源可用性）或资源成本。似乎很明显，比较自动扩展方法并不那么直接。例如，让我们考虑两个建议，一个专注于短期预测具有明显昼夜模式的工作负载，另一个则试图在流量突然爆发的情况下遵守SLA的规定。这两种自动扩展系统对其目标系统来说都很好，但显然后一种方法更容易造成过度配置。不过，如果认为这种由于过度供应状态而产生的额外成本足以让一个自动扩展系统取代另一个自动扩展系统，那是错误的。那么，自动扩展器的目标就不能作为分类标准了。
一个可能的自动扩展器分组可以使用它们的预测能力，将它们分为两类：反应式和主动式。前者意味着系统只有在检测到工作负载的变化时才会做出反应，使用从监测的变量集中获得的最后数值；因此，由于资源分配需要一些时间，预期的效果可能会在太晚的时候到来。积极主动的系统会预测未来的需求，并在考虑到这些需求的情况下做出决定。我们选择不使用这个作为分类标准，因为有时并不清楚一个特定的方法是纯粹的反应性还是主动性。
我们决定采用用于构建自动缩放器的基础理论或技术作为我们的分类标准。这将有助于读者更好地理解某一特定组别或类别的基本特征，包括其优点和局限性。大多数审查的作品都可以归入这五组中的一组或多组（注意有些作品提出了几种技术的混合）。
